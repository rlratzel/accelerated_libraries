...
# central nodes.
#
# This script uses multiple values of k to demonstrate how a higher number of
# sampled shortest paths can result in the detection of more highly-central
# nodes and improved accuracy.
...
print(f"read_csv:\t\t{time.time()-st:.4}s", flush=True)

st=time.time()
G = nx.from_pandas_edgelist(edgelist_df, create_using=nx.DiGraph)
print(f"from_pandas_edgelist:\t{time.time()-st:.4}s", flush=True)
...
for k in k_vals:
    ...
    print(f"betweenness_centrality:\t{time.time()-st:.4}s", flush=True)

    st=time.time()
    bc_values_df = pd.DataFrame(bc_values_dict.items(), columns=["node", "bc"])
    print(f"to DataFrame:\t\t{time.time()-st:.4}s", flush=True)
    ...
    print(f"\ngroupby:\t\t{time.time()-st:.4}s", flush=True)
